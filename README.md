...
* **Data-Control Decoupling & Pipeline Latency:** Suspected a critical logic flaw at the CDC boundary. Pixel data inherently suffered latency as it queued through the `camera_read` module and the Async FIFO. However, the horizontal control signal (`HREF`) completely bypassed this upstream data-path and propagated instantly to the `downscaling` module.
