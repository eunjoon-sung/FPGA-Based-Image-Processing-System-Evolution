...
* **Hypothesis (Architectural Deduction):** Without clear ILA triggers across the asynchronous boundary, an architectural hypothesis was formulated:
  1. **Clock Frequency Drift & Severe Tearing:** The precise 5-way split observed during the static color bar test strongly implied a microscopic frequency difference between the camera's external `PCLK` and the FPGA's internal system clock (`25MHz`). Because the V1 architecture lacked a Full-Frame Buffer to isolate the read/write domains, this plesiochronous mismatch caused the camera's write pointer to continuously drift and overtake the display's read pointer. This structural lack of memory decoupling manifested directly as the severe multi-line tearing (the 5-way split) on the monitor.
  2. **Data-Control Decoupling (Phase Skew):** Suspected a critical architectural flaw where the Data path and Control path were structurally decoupled across the CDC boundary. Pixel data inherently suffered pipeline latency as it queued through the Async FIFO, whereas control signals (`VSYNC`/`HREF`) bypassed the FIFO and propagated instantly. This timing mismatch caused severe phase skew: the SRAM address reset triggered prematurely based on the instant `VSYNC`, while the corresponding tail-end pixels were still trapped inside the FIFO, misaligning the pixel-to-address mapping and utterly destroying the frame structure.
