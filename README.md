...
* **Result (The Shifting Boundary & Mathematical Alignment)):** A 64-bit AXI bus transfers 8 Bytes per beat. At AWLEN = 64, one burst equals 512 Bytes. At AWLEN = 16, one burst equals 128 Bytes. When AWLEN was changed to 16, the visual folding boundary neatly aligned horizontally.
Retroactive Analysis: Knowing the RTL bug doubled the data, the video line was actually 1280 Bytes (320px * 4B). With AWLEN=64 (512B), it took exactly 2.5 bursts per line, causing the burst alignment to stagger across scanlines. However, with AWLEN=16 (128B), 1280 Bytes was perfectly divisible (exactly 10 bursts per line). This perfect mathematical alignment temporarily "stabilized" the offset, revealing the true nature of the data duplication.
