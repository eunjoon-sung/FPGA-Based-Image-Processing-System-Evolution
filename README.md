...
* **Data-Control Decoupling & Pipeline Latency:** Suspected a critical logic flaw at the CDC boundary. Pixel data inherently suffered latency as it queued through the Async FIFO and navigated the handshake registers between fragmented RTL modules (`camera_read` → `downscaling` → `sram_writer`). However, the horizontal control signal (`HREF`) completely bypassed this entire heavy data-path and propagated instantly. 
* **Spatial Phase Skew (Wrap-around):** This architectural decoupling caused a severe spatial phase skew. The Downscaler prematurely reset its `x_count` coordinate to zero on the instant `HREF` falling edge, while the corresponding tail-end pixels were still emerging from the delayed pipeline. These lingering pixels wrapped around to the beginning of the next scanline, progressively shifting the X-coordinates. This caused the decimation logic (`x_count % 2 == 0`) to sample misaligned adjacent pixels, completely destroying the image's spatial structure.
* **Vertical Beat Frequency (The 5-way Split):** Furthermore, the fractional phase drift between the 24MHz PCLK and 25MHz system clock accumulated across each scanline and synchronized into integers every 3–4 lines. This vertical beat frequency created a distinct harmonic wrap-around effect, manifesting visually as 5 static vertical pillars with repeating horizontal color banding in test patterns.
* **Data Throughput Mismatch (The Hidden Tearing):** Secondary analysis revealed that even if spatial mapping was correct, the architecture fundamentally lacked a Full-Frame Buffer. Because the Async FIFO was configured for immediate read-out without substantial capacity, the 25MHz SRAM Writer's effective throughput was inherently bottlenecked by the slower PCLK at the FIFO input. Meanwhile, the Video Timing Generator (VTG) read from the same single-port SRAM at a continuous 25MHz. This mismatch meant the faster VTG would inevitably lap the throttled write pointer, causing uncontrollable temporal Tearing.

**Action & Verification (Source-Synchronous Write-Path & Module Consolidation):** To empirically resolve these spatial and temporal phase issues, the architecture was drastically refactored. As illustrated in the Before/After diagram above, the unstable Async FIFO was completely removed. Three previously fragmented modules (`camera_read`, `downscaling`, and `sram_writer`) were aggressively consolidated into a single, unified `camera_capture` module, effectively eliminating all inter-module pipeline latency and handshake overhead. 

By driving this unified block strictly with the camera's native PCLK and explicitly gating the generated pixel data with the `HREF`/`VSYNC` signals in real-time, the data and control paths were physically and permanently coupled (Source-Synchronous Design). Finally, the Clock Domain Crossing (CDC) boundary was strategically shifted to the Dual-Port BRAM, allowing the read-side pipeline (HDMI/VTG) to safely operate on independent internal system clocks (25MHz/100MHz) without race conditions or phase skew.
